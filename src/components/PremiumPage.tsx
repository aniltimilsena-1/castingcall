    ...
    return (
        <div className="max-w-[1200px] mx-auto px-6 py-12 pb-32">
            <div className="text-center mb-16 px-4">
                ...
                <p className="text-white/70 text-lg md:text-xl max-w-2xl mx-auto uppercase tracking-tighter">Get discovered by top casting directors {isNepal ? "in Nepal & Worldwide" : "Worldwide"}.</p>
            </div>
            ...
            <div className="grid md:grid-cols-2 gap-8 mb-20">
                {plans.map((plan, idx) => (
                    <motion.div
                        ...
                        key={plan.name}
                        className={`bg-card/30 backdrop-blur-3xl border-2 rounded-3xl p-8 relative overflow-hidden flex flex-col ${plan.popular ? "border-primary" : "border-white/5"}`}
                    >
                        {plan.popular && (
                        ...
                        <div className="flex items-baseline gap-2 mb-10">
                            ...
                            <span className="text-xs text-white/75">/ PERMANENT ACCESS</span>
                        </div>
                        <ul className="space-y-5 mb-12 flex-1">
                            {plan.features.map(f => <li key={f} className="flex items-center gap-4 text-sm text-white font-normal"><Check size={16} className="text-primary" /> {f}</li>)}
                        </ul>
                        <button
            ...
            <div className="bg-card/60 backdrop-blur-3xl border border-black/10 rounded-3xl p-10 text-center max-w-4xl mx-auto shadow-2xl relative overflow-hidden">
                <div className="absolute top-0 right-0 w-64 h-64 bg-primary/5 blur-[100px] -z-10" />
                <Wallet className="text-primary w-8 h-8 mx-auto mb-8" />
                <h3 className="text-2xl text-white font-display italic mb-3">The Smart Move for Your Talent</h3>
                <p className="text-white/70 text-[0.65rem] uppercase tracking-[2px] mb-8">Trusted by over {talentsCount} talents globally across the network.</p>
...
function Benefit({ icon, label }: any) {
    return (
        <div className="flex flex-col items-center gap-3">
            ...
            <span className="text-[0.55rem] uppercase tracking-widest text-white/70">{label}</span>
        </div>
    );
}
