  ...
  const handleSubmit = async () => {
    ...
    try {
      if (tab === "login") {
        ...
        if (!signupName || !signupEmail || !signupPass || !signupRole) { toast.error("Please fill all fields"); return; }
      ...
      >
        ...
        {tab === "login" ? (
          ...
          <div className="space-y-4">
            ...
            <Field label="PASSWORD" type="password" value={signupPass} onChange={setSignupPass} placeholder="Min 6 characters" />
            <div>
  ...
  return (
    <div>
      ...
      <input
        ...
        className="w-full bg-background border-[1.5px] border-border rounded-lg px-4 py-2.5 text-foreground font-body text-sm outline-none focus:border-primary transition-colors placeholder:text-muted-foreground/40"
      />
    </div>
  );
}
