...
export default function BottomNav({ activePage, onNavigate }: BottomNavProps) {
  ...
  return (
    <nav className="md:hidden fixed bottom-0 left-0 w-full z-[400] bg-[#121826]/80 backdrop-blur-3xl border-t border-white/5 pb-safe px-6 h-18 sm:h-20 flex items-center justify-between shadow-[0_-10px_30px_rgba(0,0,0,0.4)]">
      {items.map((item) => {
        ...
        if (isPlus) {
          return (
            <button
              ...
              className="relative -top-1 w-14 h-14 bg-[#D4AF37] rounded-full flex items-center justify-center text-[#0B0F1A] shadow-[0_4px_20px_rgba(212,175,55,0.4)] transition-all active:scale-90 scale-110"
            >
        ...
        return (
          ...
          >
            <item.icon size={22} variant={isActive ? "fill" : "outline"} />
