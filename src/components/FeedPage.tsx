    ...
    useEffect(() => {
        ...
        const loadFeed = async () => {
            ...
            try {
                ...
                (profiles || []).forEach((p: any) => { // Cast p to any here
                    const owner = {
                        id: p.user_id,
                        name: p.name || "Unknown",
                ...
                try {
                    ...
                    const commentMap: Record<string, Comment[]> = {}; // Use Comment type
                    mediaUrls.forEach(url => {
                        commentMap[url] = (commentRows || [])
                            .filter((c) => c.photo_url === url)
                            .map((c) => {
                                const prof = commenterProfiles.find(cp => cp.user_id === c.user_id);
                                return {
                                    ...c,
                                    photo_url: url, // Map photo_url correctly
