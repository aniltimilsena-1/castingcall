  ...
  const fetchProfile = useCallback(async (userId: string) => {
    ...
    try {
      const { data, error } = await supabase
        .from("profiles")
        .select("id, user_id, name, email, role, bio, location, photo_url, plan, is_verified, created_at, height, age, experience_years, gender, hair_color, eye_color, portfolio_url, skills, photos, videos, phone")
        .eq("user_id", userId)
        .maybeSingle();
      ...
      if (error && error.code !== "PGRST116") {
        ...
        if (error.message?.toLowerCase().includes("jwt expired")) {
          console.warn("JWT expired during profile fetch. Retrying session refresh...");
          // supabase.auth.refreshSession() could be tried here, but standard catch will handle it.
        }
      }
